...
from simulation.citygraph_dataset import CityGraphData, CityGraphDataset, \
    ...
    DEMAND_KEY, get_dataset_from_config
#### ENDS HERE ####
from simulation.transit_time_estimator_SouthernHarbourRoutes import RouteGenBatchState
from torch_utils_2 import get_batch_tensor_from_routes
...
def train_ppo(model, min_n_routes, max_n_routes, cfg, optimizer, 
    ...
    for iteration in range(cfg.ppo.n_iterations):
        ...
        for idxs in torch.split(train_order, n_states_per_minibatch):
            ...
            ## AA - CHANGE ANDRE ##
            #### STARTS HERE (Set error_if_nonfinite to False)####
...
def setup_and_train(cfg: DictConfig, trial: optuna.trial.Trial = None):
    ...
    # load the data
    train_ds = get_dataset_from_config(cfg.eval.dataset) * cfg.batch_size
    train_dl = DataLoader(train_ds, batch_size=cfg.batch_size, shuffle = True)
    val_ds = get_dataset_from_config(cfg.eval.dataset)
    ...
    # AA - CHANGE ANDRE
    #### STARTS HERE (Changes 'val_batch_size' to 1024 ####
    #val_weights = val_cost_obj.sample_variable_weights(val_batch_size, DEVICE)
    val_weights = val_cost_obj.sample_variable_weights(2, DEVICE)
    #### ENDS HERE ####
...
@hydra.main(version_base=None, config_path="../cfg", config_name="mandl_2")
