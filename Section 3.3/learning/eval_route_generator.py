...
def sample_from_model(model, state, cost_obj, n_samples=20, 
    ...
    # AA - Print the best routes and their costs
    for idx, plan in enumerate(best_plans):
        plan_cost = all_costs[min_indices[idx]].item()  # Get the cost for each best route
        print(f"Best route {idx+1}: {plan}, Cost: {plan_cost}")    
